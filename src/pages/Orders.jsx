...
const Orders = () => {
  ...
  const cancelOrder = async (orderId) => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:8001/orders/${orderId}/cancel`, {
        method: 'POST',
        headers: {
  ...
  const retryPayment = async (orderId) => {
    ...
    try {
      const token = localStorage.getItem('access_token');
      const response = await fetch(`http://localhost:8001/orders/${orderId}/retry-payment`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
    ...
    <div className="min-h-screen bg-warm py-8 px-4">
      <div className="container mx-auto max-w-6xl">
        ...
        <div className="text-center mb-8">
          ...
          <h1 className="font-h text-3xl md:text-4xl font-bold text-black uppercase mb-2">My Orders</h1>
        ...
        {orders.length === 0 ? (
          <div className="text-center py-16 bg-white border-4 border-black shadow-hard-lg">
            ...
            <p className="font-h text-2xl font-bold text-black uppercase mb-2">No orders yet</p>
            <p className="text-ash mb-6">You haven't placed any orders yet.</p>
            <Link to="/products" className="inline-block bg-terra text-white px-8 py-3 font-bold uppercase tracking-wider border-4 border-black shadow-hard-sm hover:translate-x-[2px] hover:translate-y-[2px] transition-all">
              Start Shopping
            </Link>
          </div>
        ) : (
          <div className="space-y-6">
            {orders.map((order) => (
              <div key={order.id} className="bg-white border-4 border-black shadow-hard-sm overflow-hidden">
                <div className="p-6">
                  {/* Order Header */}
                  ...
                  {order.items && order.items.length > 0 && (
                    <div className="mb-4">
                      <h3 className="font-h font-bold text-black uppercase text-sm mb-3">Items</h3>
                  ...
                  {order.mpesa_receipt && (
                    <div className="mb-4 p-3 bg-green-50 border-2 border-green-500">
                      <p className="text-sm text-green-700">
                        M-Pesa Receipt: <span className="font-mono font-bold">{order.mpesa_receipt}</span>
                      </p>
                    </div>
                  )}
                  ...
                  <div className="border-t-2 border-black pt-4 mt-4">
                    <div className="flex flex-wrap gap-3">
                      ...
                      {(order.status === 'cancelled' || order.status === 'payment_failed') && (
                        ...
                        >
                          <FiRefreshCw className={`w-4 h-4 ${processingOrderId === order.id ? 'animate-spin' : ''}`} />
                          {processingOrderId === order.id ? 'Processing...' : 'Retry Payment'}
                        </button>
                      )}
                      ...
                      {order.status === 'pending' && (
                        ...
                        >
                          ...
                          {processingOrderId === order.id ? 'Processing...' : 'Cancel Order'}
                        </button>
                      )}
