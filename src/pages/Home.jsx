...
const Home = () => {
  ...
  const whyChooseUs = [
    ...
    { icon: <FiTruck className="w-8 h-8" />, title: "Best Prices", description: "Price match guarantee", color: "bg-terra/10 text-terra" },
  ...
  // ✅ SAFE: Ensure partners is an array before slicing
  ...
  // ✅ SAFE: Ensure products is an array before slicing
  ...
  return (
    <div className="min-h-screen bg-warm">
      ...
      <section className="relative overflow-hidden">
        <div className="relative h-[500px] md:h-[600px] overflow-hidden">
              ...
              <div className="relative h-full flex items-center">
                <div className="container mx-auto px-4">
                  <div className="max-w-2xl text-white">
                    ...
                    <Link to="/products" className="inline-flex items-center px-6 py-3 bg-terra text-white rounded-lg font-semibold hover:bg-terra-dark transition-colors group">
          ...
          <div className="absolute bottom-6 left-1/2 -translate-x-1/2 flex space-x-2">
            {heroSlides.map((_, index) => (<button key={index} onClick={() => setCurrentSlide(index)} className={`w-2 h-2 rounded-full transition-all ${index === currentSlide ? 'w-8 bg-terra' : 'bg-white/50'}`} />))}
          </div>
        </div>
      </section>
      ...
      <section className="py-16 bg-white">
        <div className="container mx-auto px-4">
          <div className="text-center mb-12">
            <h2 className="text-3xl md:text-4xl font-bold text-gray-800 mb-4">Why Choose Kione Hardware?</h2>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
            {whyChooseUs.map((item, index) => (
              <div key={index} className="text-center group hover:transform hover:scale-105 transition-all duration-300 p-6 rounded-xl bg-gray-50">
                <div className={`inline-flex p-4 ${item.color} rounded-full mb-4 group-hover:scale-110 transition-transform`}>{item.icon}</div>
        ...
        <div className="container mx-auto px-4">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
            {categories.map((category, index) => (
              <Link key={index} to="/products" className={`${category.color} rounded-xl p-6 text-center hover:shadow-lg transition-all transform hover:-translate-y-1`}>
      ...
      <section className="py-16 bg-white">
        <div className="container mx-auto px-4">
          <div className="flex justify-between items-center mb-12">
            <div><h2 className="text-3xl md:text-4xl font-bold text-gray-800 mb-2">Latest Arrivals</h2><p className="text-gray-600">Check out our newest products</p></div>
            <Link to="/products" className="text-terra hover:text-terra-dark font-semibold flex items-center group">View All <FiChevronRight className="ml-1 group-hover:translate-x-1 transition-transform" /></Link>
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {latestProducts.map((product) => (
              <div key={product.id} className="group bg-white rounded-xl shadow-md overflow-hidden hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-1" onMouseEnter={() => setHoveredProduct(product.id)} onMouseLeave={() => setHoveredProduct(null)}>
                <div className="relative overflow-hidden h-64">
                  <img src={product.image_url || `https://picsum.photos/seed/${product.id}/400/400`} alt={product.name} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500" />
                  <div className={`absolute inset-0 bg-black/50 flex items-center justify-center space-x-3 transition-opacity duration-300 ${hoveredProduct === product.id ? 'opacity-100' : 'opacity-0'}`}>
                    <button onClick={() => handleAddToCart(product)} className="p-2 bg-terra text-white rounded-full hover:bg-terra-dark transition-colors"><FiShoppingCart className="w-5 h-5" /></button>
                    <Link to={`/product/${product.id}`} className="p-2 bg-terra text-white rounded-full hover:bg-terra-dark transition-colors"><FiEye className="w-5 h-5" /></Link>
                  </div>
                  {product.stock < 10 && product.stock > 0 && (<div className="absolute top-4 left-4 bg-terra text-white px-2 py-1 rounded-md text-xs font-semibold">Low Stock</div>)}
                  <div className="absolute top-4 right-4 bg-terra text-white px-2 py-1 rounded-md text-xs font-semibold">New</div>
                </div>
                <div className="p-4">
                  <h3 className="text-lg font-semibold text-gray-800 mb-2 line-clamp-1">{product.name}</h3>
                  <p className="text-gray-600 text-sm mb-3 line-clamp-2">{product.description}</p>
                  <div className="flex justify-between items-center">
                    <span className="text-2xl font-bold text-terra">KSh {product.price?.toLocaleString() || 0}</span>
                    {addedToCart[product.id] && <span className="text-green-600 text-sm font-semibold animate-bounce">Added!</span>}
                  </div>
                  <div className="flex items-center mt-2">
                    {[...Array(5)].map((_, i) => (<FiStar key={i} className={`w-4 h-4 ${i < 4 ? 'text-yellow-400 fill-current' : 'text-gray-300'}`} />))}
          ...
          <div className="max-w-4xl mx-auto">
            <div className="relative bg-white rounded-2xl shadow-xl p-8 md:p-12">
              ...
              <div className="flex justify-center mb-6">
                {[...Array(testimonials[currentTestimonial].rating)].map((_, i) => (<FiStar key={i} className="w-6 h-6 text-yellow-400 fill-current" />))}
              </div>
              <p className="text-gray-700 text-lg md:text-xl text-center italic mb-8 leading-relaxed">"{testimonials[currentTestimonial].text}"</p>
              <div className="flex flex-col items-center text-center">
                ...
                <p className="text-gray-400 text-xs mt-1">{testimonials[currentTestimonial].date}</p>
              </div>
              <button onClick={prevTestimonial} className="absolute left-2 top-1/2 -translate-y-1/2 bg-white shadow-lg p-2 rounded-full hover:bg-terra hover:text-white transition-all"><FiChevronLeft className="w-6 h-6" /></button>
              <button onClick={nextTestimonial} className="absolute right-2 top-1/2 -translate-y-1/2 bg-white shadow-lg p-2 rounded-full hover:bg-terra hover:text-white transition-all"><FiChevronRight className="w-6 h-6" /></button>
            </div>
          </div>
        </div>
      </section>
          ...
          <div className="relative">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
              {visiblePartners.map((partner) => (
                <a key={partner.id} href={partner.website} target="_blank" rel="noopener noreferrer" className="group bg-gray-50 rounded-xl p-6 text-center hover:shadow-lg transition-all duration-300 hover:-translate-y-1">
            ...
            {partners.length > partnersPerView && (
              <>
                <button onClick={prevPartners} className="absolute left-0 top-1/2 -translate-y-1/2 -ml-4 bg-white shadow-lg p-2 rounded-full hover:bg-terra hover:text-white transition-all"><FiChevronLeft className="w-5 h-5" /></button>
                <button onClick={nextPartners} className="absolute right-0 top-1/2 -translate-y-1/2 -mr-4 bg-white shadow-lg p-2 rounded-full hover:bg-terra hover:text-white transition-all"><FiChevronRight className="w-5 h-5" /></button>
              </>
            )}
          </div>
        </div>
          ...
          <div className="container mx-auto px-4 text-center">
            <div className="max-w-3xl mx-auto">
              ...
              <div className="flex flex-col sm:flex-row gap-4 justify-center">
                <Link to="/register" className="px-8 py-3 bg-white text-terra rounded-lg font-semibold hover:bg-gray-100 transition-colors">Create Account</Link>
                <Link to="/login" className="px-8 py-3 bg-transparent border-2 border-white text-white rounded-lg font-semibold hover:bg-white hover:text-terra transition-colors">Login</Link>
              </div>
            </div>
          </div>
        </section>
      ...
      <div className="fixed bottom-6 left-6 z-50 group">
        <a href="https://wa.me/254714391137?text=Hello%21%20I%20have%20a%20question%20about%20your%20products" target="_blank" rel="noopener noreferrer" className="flex items-center justify-center w-14 h-14 bg-terra rounded-full shadow-lg hover:bg-terra-dark transition-all duration-300 hover:scale-110">
