...
const OrderDetail = () => {
  ...
  const fetchOrder = async () => {
    try {
      ...
      const response = await fetch(`http://localhost:8001/orders/${id}`, {
        headers: { 
    ...
    return (
      <div className="min-h-screen bg-warm flex items-center justify-center">
        <div className="text-center">
          ...
          <Link to="/orders" className="text-terra hover:underline">Back to Orders</Link>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-warm py-8 px-4">
      <div className="container mx-auto max-w-4xl">
        {/* Back Button */}
        <Link to="/orders" className="inline-flex items-center gap-2 text-terra hover:text-terra-dark mb-6 group">
        ...
        <div className="bg-white border-4 border-black shadow-hard-lg overflow-hidden mb-6">
          <div className="bg-terra/10 p-6 border-b-4 border-black">
            <div className="flex flex-wrap justify-between items-center gap-4">
              <div>
                <h1 className="font-h text-3xl font-bold text-black uppercase">Order #{order.id}</h1>
              ...
              <div className="flex items-center gap-3">
                ...
                <span className={`px-4 py-2 text-sm font-bold uppercase border ${getStatusColor(order.status)}`}>
          ...
          <div className="p-6 border-b-4 border-black">
            <h2 className="font-h text-xl font-bold text-black uppercase mb-4">Order Status</h2>
            <div className="flex flex-wrap justify-between">
              <div className={`text-center ${order.created_at ? 'opacity-100' : 'opacity-50'}`}>
                ...
                {order.created_at && <p className="text-xs text-ash mt-1">{new Date(order.created_at).toLocaleDateString()}</p>}
              </div>
              <div className="flex-1 h-0.5 bg-gray-300 self-start mt-5 mx-2"></div>
              <div className={`text-center ${order.paid_at ? 'opacity-100' : 'opacity-50'}`}>
                <div className="w-10 h-10 rounded-full bg-terra/20 border-2 border-terra flex items-center justify-center mx-auto mb-2">
                  <FiCheckCircle className="w-5 h-5 text-terra" />
                </div>
                <p className="text-xs font-bold">Payment</p>
                {order.paid_at && <p className="text-xs text-ash mt-1">{new Date(order.paid_at).toLocaleDateString()}</p>}
              </div>
              <div className="flex-1 h-0.5 bg-gray-300 self-start mt-5 mx-2"></div>
              <div className={`text-center ${order.status === 'shipped' ? 'opacity-100' : 'opacity-50'}`}>
                <div className="w-10 h-10 rounded-full bg-terra/20 border-2 border-terra flex items-center justify-center mx-auto mb-2">
          ...
          {/* Order Items */}
          <div className="p-6 border-b-4 border-black">
            <h2 className="font-h text-xl font-bold text-black uppercase mb-4">Order Items</h2>
            <div className="space-y-3">
              {order.items?.map((item, idx) => (
                <div key={idx} className="flex justify-between items-center p-4 bg-warm/30 border-2 border-black">
                  <div className="flex-1">
                    <p className="font-h font-bold text-black">{item.product_name || `Product #${item.product_id}`}</p>
                    <p className="text-sm text-ash">Quantity: {item.quantity}</p>
                  </div>
                  <div className="text-right">
                    <p className="font-bold text-terra">{formatMoney(item.price)}</p>
                    <p className="text-sm text-ash">each</p>
                  </div>
                  <div className="text-right ml-6">
                    <p className="font-bold text-black">Total</p>
                    <p className="font-bold text-terra">{formatMoney(item.price * item.quantity)}</p>
                  </div>
                </div>
              ))}
            </div>
          ...
          {/* Payment Information */}
          <div className="p-6 border-b-4 border-black">
            <h2 className="font-h text-xl font-bold text-black uppercase mb-4">Payment Information</h2>
            <div className="grid md:grid-cols-2 gap-4">
              ...
              {order.mpesa_receipt && (
                <div className="md:col-span-2 p-3 bg-green-50 border-2 border-green-500">
                  <p className="text-sm text-green-700">M-Pesa Receipt Number</p>
                  <p className="font-mono font-bold text-green-800">{order.mpesa_receipt}</p>
                </div>
              )}
              {order.paid_at && (
                <div className="md:col-span-2 p-3 bg-gray-50 border-2 border-black">
                  <p className="text-sm text-ash">Payment Date</p>
                  <p className="font-bold text-black">{new Date(order.paid_at).toLocaleString()}</p>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="p-6">
            <h2 className="font-h text-xl font-bold text-black uppercase mb-4">Customer Information</h2>
            <div className="grid md:grid-cols-2 gap-4">
